...
class LossMetric(tf.keras.metrics.Metric):
  ...
  def __init__(
    ...
    if isinstance(loss_fn, tf.keras.metrics.Metric):
      name = loss_fn.name
      dtype = loss_fn.dtype
    ...
    if isinstance(loss_fn, tf.keras.metrics.Metric):
      ...
      loss_metric = tf.keras.metrics.Mean(name=name, dtype=dtype)
  ...
  def update_state(
    ...
    if isinstance(self._loss_fn, tf.keras.metrics.Metric):
