...
class LossMetricTest(keras_test_case.KerasTestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      },
      {
          ...
          "loss_fn": tf.keras.metrics.MeanSquaredError(name="loss"),
          "from_logits": False,
          "y_true": tf.constant([[0], [0], [2], [2]]),
          "y_pred": tf.constant([[1], [2], [3], [4]]),
          "is_treatment": tf.constant([[True], [False], [True], [False]]),
      ...
      },
      {
          ...
          "loss_fn": tf.keras.metrics.MeanAbsoluteError(name="loss"),
          "from_logits": False,
          "y_true": tf.constant([[0], [0], [2], [7]]),
          "y_pred": tf.constant([[1], [2], [3], [4]]),
          "is_treatment": tf.constant([[True], [False], [True], [False]]),
      ...
      },
      {
          ...
          "loss_fn": tf.keras.metrics.mean_absolute_error,
          "from_logits": False,
          "y_true": tf.constant([[0], [1], [5]]),
          "y_pred": tf.constant([[1], [2], [5]]),
      ...
      },
      {
          ...
          "loss_fn": tf.keras.metrics.BinaryCrossentropy(name="loss"),
          "from_logits": False,
          "y_true": tf.constant([[]]),
          "y_pred": tf.constant([[]]),
          "is_treatment": tf.constant([[]]),
      ...
      },
      {
          ...
          "loss_fn": tf.keras.metrics.AUC(from_logits=True, name="loss"),
          "from_logits": True,
      ...
      },
      {
          ...
          "loss_fn": tf.keras.metrics.BinaryCrossentropy(from_logits=False),
          "from_logits": False,
          "y_true": tf.constant([[0.0, 1.0]]),
          "y_pred": tf.constant([[0.0, 1.0]]),
          "is_treatment": tf.constant([[0], [0]]),
  ...
  def test_metric_with_y_pred_tensor(self):
    ...
    metric = loss_metric.LossMetric(
        loss_fn=tf.keras.metrics.mae, slice_by_treatment=False
    )
  ...
  def test_metric_is_configurable(
      self,
      loss_fn: (
          Callable[[tf.Tensor, tf.Tensor], tf.Tensor] | tf.keras.metrics.Metric
      ),
  ...
  def test_slice_by_treatment_with_y_pred_tensor_raises_error(self):
    metric = loss_metric.LossMetric(
        tf.keras.metrics.mae, slice_by_treatment=True
    )
    with self.assertRaisesRegex(
        ValueError,
  ...
  def test_passing_loss_object_raises_error(self):
    with self.assertRaisesRegex(
      ...
      loss_metric.LossMetric(loss_fn=tf.keras.losses.MeanAbsoluteError())

  def test_conflicting_from_logits_values_raises_error(self):
    with self.assertRaises(ValueError):
      loss_metric.LossMetric(
          loss_fn=tf.keras.metrics.BinaryCrossentropy(from_logits=True),
          from_logits=False,
      )
