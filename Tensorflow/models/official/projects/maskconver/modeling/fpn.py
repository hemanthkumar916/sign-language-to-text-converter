...
class MaskConverFPN(tf.keras.Model):
  ...
  def __init__(
    ...
    if use_layer_norm:
      norm_layer = lambda: tf.keras.layers.LayerNormalization(epsilon=1e-6)
    else:
...
def build_fpn_decoder(
  ...
  Returns:
    A `tf.keras.Model` instance of the MaskConverFPN decoder.
