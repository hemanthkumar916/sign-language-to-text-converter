...
class MaskConverHead(tf.keras.layers.Layer):
  ...
  def build(self, input_shape: Union[tf.TensorShape, List[tf.TensorShape]]):
    ...
    if self._config_dict['use_layer_norm']:
      bn_layer = lambda: tf.keras.layers.LayerNormalization(epsilon=1e-6)
    else:
      bn_kwargs = {
          'axis': self._bn_axis,
      ...
      if self._config_dict['use_sync_bn']:
        bn_layer = lambda: tf.keras.layers.experimental.SyncBatchNormalization(  # pylint: disable=g-long-lambda
            **bn_kwargs)
      else:
        bn_layer = lambda: tf.keras.layers.BatchNormalization(**bn_kwargs)
...
class MLP(tf.keras.Model):
  ...
  def __init__(
    ...
    bn_layer = lambda: tf.keras.layers.LayerNormalization(epsilon=1e-6)
    self.dense = [
        tf.keras.layers.Dense(d, kernel_regularizer=l2_regularizer)
