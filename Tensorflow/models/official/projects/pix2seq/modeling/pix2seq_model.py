...
class Pix2Seq(tf.keras.Model):
  ...
  def __init__(
      self,
      backbones: Sequence[tf.keras.Model],
    ...
    self._encoder_dropouts = [
        tf.keras.layers.Dropout(r) for r in encoded_feature_dropout_rates
    ]
    ...
    self._stem_projections = [
        tf.keras.layers.Dense(self._hidden_size, name="stem_projection")
        for _ in range(num_backbones)
    ]
    self._stem_lns = [
        tf.keras.layers.LayerNormalization(epsilon=1e-6, name="stem_ln")
        for _ in range(num_backbones)
    ]
  ...
  @property
  def backbones(self) -> Sequence[tf.keras.Model]:
  ...
  def checkpoint_items(
      self,
  ) -> Mapping[str, Union[tf.keras.Model, tf.keras.layers.Layer]]:
...
class Pix2SeqTransformer(tf.keras.layers.Layer):
  ...
  def __init__(
    ...
    self._output_ln_encs = [
        tf.keras.layers.LayerNormalization(epsilon=1e-6, name="output_ln_enc")
        for _ in range(self._num_sources)
    ]

    self._projs = [
        tf.keras.layers.Dense(self._hidden_size, name="proj/linear")
        for _ in range(self._num_sources)
    ]
    self._proj_lns = [
        tf.keras.layers.LayerNormalization(epsilon=1e-6, name="proj/ln")
        for _ in range(self._num_sources)
    ]
    self._proj_mlps = [
        transformer.MLP(
