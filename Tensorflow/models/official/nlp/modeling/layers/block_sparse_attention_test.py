...
class BlockSparseAttentionTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_default_masked_attention(
    ...
    # Create a 3-dimensional input (the first dimension is implicit).
    batch_size = 3
    query = tf.keras.Input(shape=(seq_len, 8))
    value = tf.keras.Input(shape=(seq_len, 8))
    ...
    # Tests the layer with three inputs: Q, K, V.
    key = tf.keras.Input(shape=(seq_len, 8))
    output = test_layer(
        query, value=value, key=key, attention_mask=mask_tensor
    )
    model = tf.keras.Model([query, value, key, mask_tensor], output)
  ...
  def test_initializer(self):
    ...
    self.assertNotAllClose(
        tf.keras.backend.eval(test_layer._query_dense.kernel),
        tf.keras.backend.eval(test_layer._key_dense.kernel),
    )
    self.assertNotAllClose(
        tf.keras.backend.eval(test_layer._query_dense.kernel),
        tf.keras.backend.eval(test_layer._value_dense.kernel),
    )
    self.assertNotAllClose(
        tf.keras.backend.eval(test_layer._query_dense.kernel),
        tf.keras.backend.eval(test_layer._output_dense.kernel),
    )
  ...
  def test_sublayer_dtypes(self, dtype):
    ...
    query = tf.keras.Input(shape=(40, 80), dtype=dtype)
    # Build the layer
  ...
  def test_dropout(self):
    ...
    self.assertNotAllClose(
        tf.keras.backend.eval(train_out), tf.keras.backend.eval(test_out)
    )
  ...
  def test_query_mask_progagation(self):
    ...
    masked_query = tf.keras.layers.Embedding(4, 8, mask_zero=True)(query)
  ...
  def test_value_mask(self):
    ...
    masked_value = tf.keras.layers.Embedding(6, 8, mask_zero=True)(value)
    output = test_layer(
        query=masked_query,
        value=masked_value,
    )
